...
function generateReport() {
  ...
  async.auto({
      ...
      saveCss: ['createDirs', 'parseLess', function(callback,  results) {
        ...
        saveToFile(results.parseLess, config.reportCssFile, callback);
      }],
      saveJs: ['createDirs', 'minifyJs', function(callback, results) {
        // console.log('saveJs');
        // after creating directories and minifying js,
        // save the js to file
        saveToFile(results.minifyJs.code, config.reportJsFile, callback);
      }],
...
function saveFonts(callback) {
  ncp(config.bsFontsDir, config.reportFontsDir, function (err) {
    if (err) callback(err);
    callback(null, 'done');
  });
}
