...
var splitChar = process.platform === "win32" ? '\\' : '/',
    ...
    fontsDir = path.join(nodeModulesDir, 'bootstrap', 'fonts'),
    reportsDir = path.join(process.cwd(), 'mochawesome-reports'),
    reportsJsDir = path.join(reportsDir, 'js'),
    reportsFontsDir = path.join(reportsDir, 'fonts'),
    reportsCssDir = path.join(reportsDir, 'css'),
    ...
    reportCssFile = path.join(reportsCssDir, 'mochawesome.css'),
...
function createStylesheet () {
  ...
  var bsPath = path.join(nodeModulesDir, 'bootstrap', 'less');
...
/**
 * Copy fonts into reports directory.
 *
 * @api private
 */
...
function saveFonts() {
  fs.readdir(fontsDir, function (err, files) {
    ...
    files.forEach(function (file) {
      var filePath = path.join(fontsDir, file);
      fs.stat(filePath, function (err, stats) {
        if (err) throw err;
        if (!stats.isDirectory()) {
          fs.readFile(filePath, {encoding:'utf8'}, function (err, data) {
            if (err) throw err;
            var fileOutPath = path.join(reportsFontsDir, file);
            fs.writeFile(fileOutPath, data, function (err) {
              if (err) throw err;
              util.print("Saved " + fileOutPath + "\n");
            });
          });
        }
      });
...
function saveJsReqs () {
  var jsReqs = [
    ...
    path.join(nodeModulesDir, 'bootstrap', 'dist', 'js', 'bootstrap.js')
  ];
...
/**
 * Create required reports directories if they do not already exist
 *
...
function createDirs () {
  var dirs = [reportsDir, reportsJsDir, reportsFontsDir, reportsCssDir];
