  ...
  runner.on('end', function () {
    try {
      if (!endCalled) {
        ...
        obj.allTests.forEach( function (test) {
          test.err = cleanErr(test.err);
        });

        obj.allPasses.forEach( function (test) {
          test.err = cleanErr(test.err);
        });

        obj.allFailures.forEach( function (test) {
          test.err = cleanErr(test.err);
        });
...
/**
 * Clone error objects to avoid circ references
 *
 * @param {Object} err
 * @api private
 */
